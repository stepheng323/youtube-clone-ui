...
function PlayVideo() {
  ...
  const { result: videoDetails, isLoading: detailsLoading } = UseFetch(
    videoDetailsUrl
  );
  const { result: videoCount, isLoading: countLoading } = UseFetch(
    videoCountUrl
  );
  const {
    result: subscriptionStatus,
    isLoading: subscriptionStatusLoading,
    setResult: setSubscriptionStatus,
  } = UseFetch(subscribersStatusUrl);
  const {
    result: channelSubscriptionCount,
    isLoading: subscriptionCountLoading,
    setResult: setSubscriptionCount,
  } = UseFetch(subscriptionCountUrl);
  const {
    result: likeCount,
    isLoading: likeCountLoading,
    setResult: setLikeCount,
  } = UseFetch(likeCountUrl);
  const {
    result: dislikeCount,
    isLoading: dislikeCountLoading,
    setResult: setDislikeCount,
  } = UseFetch(dislikeCountUrl);
  const {
    result: feeling,
    isLoading: feelingLoading,
    setResult: setFeeling,
  } = UseFetch(feelingUrl);

  const {
    handleFetch: likeVideo,
    result: likeVideoResult,
    isSubmitting: likeLoading,
  } = UseForm({}, likeVideoUrl, { method: 'POST' });
  const {
    handleFetch: dislikeVideo,
    result: dislikeVideoResult,
    isSubmitting: dislikeLoading,
  } = UseForm({}, dislikeVideoUrl, { method: 'POST' });
  ...
  const getComments = async () => {
    try {
      const response = await fetch(
        `${REACT_APP_DEV_BASE_URL}/comment/${id}?page=1&limit=8`
      );
  ...
  if (detailsLoading)
    return <div className="play-video-loading">Loading...</div>;
  // || countLoading || commentLoadng || subscriptionStatusLoading ||subscriptionCountLoading || likeCountLoading || feelingLoading || dislikeCountLoading
  const {
    title,
    viewsCount,
    description,
    createdAt,
    channel: { name: channelName, _id: channelId, channelAvatar, owner },
  ...
  const handleLike = () => {
    if (feeling.payload.like) {
      ...
      return setFeeling({
        ...feeling,
        payload: { like: false, dislike: false, hasFeeling: false },
      });
    }
    ...
    setFeeling({
      ...feeling,
      payload: { like: true, dislike: false, hasFeeling: true },
    });
  };
  ...
  const handleDislike = () => {
    ...
    setFeeling({
      ...feeling,
      payload: { like: false, dislike: true, hasFeeling: true },
    });
  };
        ...
        <div className="video-info-details">
          ...
          <div className="icons">
            <div onClick={() => handleLike()} className="icon-container">
              <ThumbUpIcon
                className={`icon ${feeling?.payload?.like && 'highlight-icon'}`}
              />
            ...
            <div onClick={() => handleDislike()} className="icon-container">
              <ThumbDownIcon
                className={`icon ${
                  feeling?.payload?.dislike && 'highlight-icon'
                }`}
              />
