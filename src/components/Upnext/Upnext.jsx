...
function Upnext() {
	...
	const fetchNext = async() => {
      ...
      const getMoreVideos = async () => {
        const response = await fetch(`${REACT_APP_DEV_BASE_URL}/video/upnext?page=${page + 1}&limit=8`);
	...
	return (
      ...
      >
			{videos.length ? videos.map((item) => {
			 const {_id, viewsCount, thumbnail, title, createdAt, channel: {name, channelAvatar}} = item;
					return (
						<SideVideoRow
						key={_id}
						id={_id}
